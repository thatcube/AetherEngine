...
├── AetherEngine+Subtitles.swift             Embedded + external subtitle pipeline (side demuxer task, cue apply / prune, external track registry + unified selection routing, #88). Duplicate recovery re-arms for the same stream and anchor coalesce instead of cancelling a healthy start mid-positioning, and a successor aborts its predecessor's wedged positioning read at handoff, so a fast-forward recovery no longer pays drain-timeout + cold reopen + a second full seek budget in series (#112 round 11)
...
├── Demuxer/
│   ├── AVIOProvider.swift                   Internal seam over a custom-AVIO byte source; AVIOReader and CustomIOReaderBridge both plug into the Demuxer through it, incl. the bounded-seek read deadline, the resolved byte size backing the byte-estimate seek fallback, and the reversible cross-thread read abort a successor side reader uses to unwedge a predecessor's in-flight positioning seek without tearing the provider down (#112 round 11)
