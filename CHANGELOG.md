...
- **PGS subtitles scrolled through the lead-in while paused after a seek or audio switch (#112).** The reconstruction pass seeks a lead-in back of the playhead and decodes forward, and the stale-arrival gate published every self-contained composition (Acquisition Point / Epoch Start) in that window the instant it decoded. A Blu-ray has several dialogue lines in a 24 s lead-in, so each replaced the last on screen: with the playhead frozen after an audio switch the overlay scrolled through ~24 s of earlier subtitles (ijuniorfu: "the subtitles keep changing while paused", "content from before the current playback position"). The gate now holds the lead-in's compositions and emits only the single line active at the playhead, once the decode reaches it, so the earlier lines never reach the overlay.
- **PGS subtitles blank after a seek or audio switch on a remote MPEG-TS whose length was starved at open (#112).** The subtitle side demuxer opens the raw origin, and `avformat_seek_file` on an MPEG-TS needs a known byte size (its timestamp binary search reads the tail). The video producer resolves that size from its persistent open, but a side demuxer opening the same origin under the producer's concurrent load could have its own size probe answered without a length (or 429'd), collapsing it to forward-only streaming where every backward seek returns -1 and reconstruction reads zero packets (ijuniorfu: "the subtitles aren't showing up"). Resolved lengths are now shared across demuxer opens of the same origin (`SourceContentLengthCache`), so a starved side demuxer reuses the producer's length and stays byte-seekable. A genuinely length-less source (live remux) never populates the cache and correctly stays streaming.
- **PGS subtitles not re-armed after a fast-forward that restarts the producer (#112).** A fast-forward whose target is outside the producer's cache range restarts the producer (an out-of-range segment fetch) instead of landing through `seek(to:)`, so `rearmEmbeddedSubtitleReaders` never ran: the PGS side reader was torn down and not replaced, and subtitles stayed gone until the next reload. The embedded bitmap reader now has the same debounced re-anchor the native mov_text path already had, fired from the producer-restart settle (never emitted for an ordinary in-budget seek). After the restart settles it re-arms the side reader at the true playhead unless the retained store already covers it.
- **PGS subtitle re-arm anchored behind the playhead after a fast-forward, then an audio switch (#112).** An audio switch does a `keepNativeHost` reload and re-arms the PGS side reader at the playhead. It read `clock.sourceTime`, which on the native path is written only by the `$renderedTime` sink and discrete seek landings, never by the clock tick (issue #49); after a fast-forward that landed via a producer restart it stayed pinned at the fast-forward's landing PTS while `currentTime` moved on. The device log showed the switch at `currentTime` 1292.3 s (true source ~1304 s) re-arming at 1211.7 s, ~92 s behind, so the reader reconstructed a region already passed (nothing showed) and flooded stale cues. The reload now maps the resume position onto the source axis (the same mapping the seek landing uses) instead of reading the possibly-stale stored `sourceTime`.
